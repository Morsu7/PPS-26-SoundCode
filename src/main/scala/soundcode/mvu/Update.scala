...
object Update:
  def update(model: AppModel, msg: Msg): (AppModel, Cmd) =
    msg match
      ...
      case Msg.CodeParsed(streams, tempo, visualizers, None) => (model.copy(errors=None, tempo=tempo.getOrElse(Tempo.default)), Cmd.UpdateTimeline(streams, tempo.getOrElse(Tempo.default), visualizers))
